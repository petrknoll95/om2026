...
export default function NavMenu() {
  ...
  return (
    <nav className="fixed top-0 left-0 flex justify-between items-center p-4 z-50">
      ...
      <div
        ref={placeholderRef}
        className={`absolute rounded-[32px] border border-dashed border-foreground/80 transition-opacity duration-500 ease-out ${isDragging ? (isNearOrigin ? "opacity-80" : "opacity-30") : "opacity-0"}`}
        style={{
      ...
      <motion.div
        ref={scope}
        className="group relative flex flex-col p-1.5 select-none"
        onMouseMove={handleMouseMove}
        style={{ x, y, scale, willChange: "transform", transform: "translateZ(0)" }}
      >
        {/* Background - adapts to container size */}
        <div
          className={`absolute inset-0 group-hover:-inset-1 -z-10 transition-all duration-500 ${isOpen && isMobile ? "rounded-[20px]" : "rounded-[32px]"}`}
          style={{
        ...
        >
          <div
            className={`absolute inset-0 shadow-[inset_0_0_6px_10px_rgba(255,255,255,0.75)] dark:shadow-[inset_0_0_12px_0px_rgba(255,255,255,0.2)] transition-[border-radius] duration-300 ${isOpen && isMobile ? "rounded-[20px]" : "rounded-[32px]"}`}
            style={{
              transform: "translateZ(0)",
            }}
          />
          <div
            className={`absolute inset-0 bg-[rgba(200,200,200,0.35)] shadow-[inset_0_0_1px_1px_rgba(255,255,255,0.25)] dark:shadow-[inset_0_0_1px_1px_rgba(255,255,255,0.05)] dark:bg-[rgba(200,200,200,0.15)] transition-[border-radius] duration-300 ${isOpen && isMobile ? "rounded-[20px]" : "rounded-[32px]"}`}
            style={{
        ...
        {/* Shimmer effect */}
        <div
          className={`absolute inset-0 group-hover:-inset-1 z-10 opacity-0 group-hover:opacity-100 dark:group-hover:opacity-50 transition-all duration-300 overflow-hidden ${isOpen && isMobile ? "rounded-[20px]" : "rounded-[32px]"}`}
        ...
        {/* Header row - logo, desktop links, toggle button */}
        ...
        {/* Mobile menu - expands within container */}
        <AnimatePresence initial={false}>
          {isOpen && isMobile && (
            <motion.div
